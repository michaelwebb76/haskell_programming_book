...
module HaskellProgrammingChapter11 where
-- Exercises: Dog types
-- 1. Type constructor
-- 2. * -> *
-- 3. *
...
capitalizeWords ""              = []
capitalizeWords string@(x : xs) = (firstWord, capitalizedFirstWord)
...
buttonConfigWithChar daPhone char = find
...
mostPopularLetter string = fst
    (maximumBy (\x y -> compare (snd x) (snd y)) charCounts)
    where charCounts = map (\x -> (x, count (\y -> y == x) string)) string

coolestLtr :: [String] -> Char
coolestLtr = mostPopularLetter . concat

coolestWord :: [String] -> String
coolestWord stringList = fst
